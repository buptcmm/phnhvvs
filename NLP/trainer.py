...
def train(args, lamda, device):
  ...
  for epoch in epoch_iter:
      ...
      for i, (x_batch, y_batch) in enumerate(train_loader):
          ...
          for j in range(n_mo_sol):
            ...
            line_direction = torch.ones_like(loss_mean)

            distance = point_to_line_distance(loss_mean, rays[j], line_direction)
      ...
      hnet.eval()
      hv, val_loss_epoch, val_min_penalty = evaluate_hv(num_ray, start, end, hnet, net,ref_point, valid_loader, emb, device)
      if best_hv < hv:
          best_hv = hv
          torch.save(hnet, 'save_models/NLP_MultiSample1_' + args.part_text + str(head) + '_' + str(lamda_o)  +".pt")
  ...
  file_path = "/root/cmm/mhe/NLP/save_models/NLP_MultiSample1_" + args.part_text + str(
      head) + "_" + str(lamda_o) + ".pt"
  hnet = torch.load(file_path)

  # hnet = torch.load('save_models/NLP_MultiSample_' + args.part_text + str(head) + '_' + str(lamda)+".pt")

  hv_epoch, test_loss, test_min_penalty = evaluate_hv(50, start, end, hnet, net,ref_point,test_loader, emb, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-folder', type=str, default='/root/cmm/mhe/NLP/input/', help='path to data')
    ...
    parser.add_argument('--lamda', type=float, default=1, help='weight decay')#0.5
