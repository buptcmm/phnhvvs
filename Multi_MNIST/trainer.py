...
def train(device: torch.device):
  ...
  for epoch in epoch_iter:
    ...
    if hv > best_hv:
      best_hv = hv
      torch.save(hnet, Path(args.out_dir) / (args.data +"_MultiSample1_" + args.part_text +  str(head) + "_" + str(lamda)+ "_.pt"))
      id = dem
      patience = 0
...
def save_results(args, test_loader, net, lamda, head, start, end):
    file_path = "/root/cmm/mhe/Multi_MNIST/save_models/" + args.data + "_MultiSample1_" + args.part_text + str(
        head) + "_" + str(lamda) + "_.pt"
    ...
    plt.show()
    print(results)
    # np.save('fronts/' + args.data + "_MultiSample1_" + args.part_text +  str(head) + "_" + str(lamda) + "_front.npy", results)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=0, help='num. epochs')
    ...
    parser.add_argument('--lamda', type=float, default=1, help='weight decay')#or 0.5
